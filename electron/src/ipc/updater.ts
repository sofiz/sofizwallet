...
const updateEndpoint = !isDev ? "https://update.sofizwallet.io/" : process.env.UPDATE_ENDPOINT
...
function getUpdaterOptions() {
  ...
  const headers = {
    "user-agent": `SatoshiPaysofiz/${app.getVersion()} ${os.platform()}/${os.release()}`,
...
async function startUpdating(version: string) {
  ...
  const response = dialog.showMessageBoxSync({
    ...
    cancelId: 1,
    defaultId: 0,
    title: "Restart the sofiz app",
    message: "sofiz needs to quit and re-open to apply the update."
  })
...
function showUpdateNotification(version: string) {
  const notification = new Notification({
    title: `New version ${version} of sofiz available`,
    body: "",
